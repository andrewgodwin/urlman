...
def test_urlstring_standalone():
    assert (
        urlman.UrlString("/posts/").full(scheme="https", hostname="localhost")
...
def test_full_hostname_url(post):
    assert (
        post.urls.view.full(hostname="127.0.0.1")
        == "http://127.0.0.1/posts/hello-world/"
    )
...
def test_full_params_url(post):
    assert (
        post.urls.view.full(params="param=1")
        == "http://localhost/posts/hello-world/;param=1"
    )


def test_full_query_url(post):
    assert (
        post.urls.view.full(query="query=1")
        == "http://localhost/posts/hello-world/?query=1"
    )


def test_full_fragement_url(post):
    assert (
        post.urls.view.full(fragment="fragment")
        == "http://localhost/posts/hello-world/#fragment"
    )
